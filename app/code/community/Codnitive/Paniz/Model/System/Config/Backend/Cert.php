...
class Codnitive_Paniz_Model_System_Config_Backend_Cert extends Mage_Core_Model_Config_Data
{
	...
	protected function _beforeSave()
	{
		...
		if ($value) {
			try {
				...
				if ($condition) {
					$err = Mage::helper('paniz')->__('%s extension not registered.',
							Mage::helper('paniz')->__(Codnitive_Paniz_Model_Config::EXTENSION_NAME));
				...
				try {
					...
					curl_setopt($ch, CURLOPT_URL, Mage::helper('paniz')->getConUrl());
				...
				if (false === $data) {
					$err = Mage::helper('paniz')->__('%s save error. Please try again later.',
							Mage::helper('paniz')->__(Codnitive_Paniz_Model_Config::EXTENSION_NAME));
					Mage::throwException($err);
				}
				if ('1' !== $data) {
					$err = Mage::helper('paniz')->__($fieldConf->label)
						. Mage::helper('paniz')->__(' or registration information for')
						. Mage::helper('paniz')->__(' %s', Mage::helper('paniz')->__(Codnitive_Paniz_Model_Config::EXTENSION_NAME))
						. Mage::helper('paniz')->__(' is not valid. Please check them or %s', Mage::helper('paniz')->__('contact us.'));
					Mage::throwException($err);
				}
			}
			catch (Exception $e) {
